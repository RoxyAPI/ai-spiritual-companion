...
/**
 * The list that turns a tool result into a drawing.
 *
 * The failure modes worth guarding are the silent ones: a tool nothing draws, a failed call, and a
 * result that is not JSON must each leave the written answer standing rather than throwing inside
 * the render. The name a vendor hands back is the only key, and a host is free to prefix it with
 * the server it came from, so that form is pinned too.
 */
...
const tarotOutput = { content: [{ type: 'text', text: JSON.stringify(tarotResult) }] };
...
describe('toolWidgetsFor', () => {
  ...
  it('resolves a tool name prefixed with the server it came from', () => {
    const widgets = toolWidgetsFor(message([toolPart('roxy_tarot:post_tarot_daily', tarotOutput)]));
  ...
  it('keeps the drawing beside the prose of the same message', () => {
    const widgets = toolWidgetsFor(
      message([
  ...
  // The message object is replaced on every streamed chunk. Parsing again each time would hand the
  // component a new object and redraw the whole chart under the arriving prose, so the same call
  // must always yield the same result object.
